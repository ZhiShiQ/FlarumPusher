...
namespace ZhiShiQ\Flarum\Pusher\Api\Controller;
...
class AuthController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if (array_get($body, 'channel_name') === $userChannel) {
            $pusher = new Pusher(
                $this->settings->get('zhishiq.pusher.app_key'),
                $this->settings->get('zhishiq.pusher.app_secret'),
                $this->settings->get('zhishiq.pusher.app_id'),
                ['cluster' => $this->settings->get('zhishiq.pusher.app_cluster')]
            );
